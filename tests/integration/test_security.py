...
def test_tampered_jwt_results_401(client: TestClient, monkeypatch: MonkeyPatch) -> None:
    ...
    assert res.status_code == 401  # invalid auth returns 401 Unauthorized


def test_revoked_token_results_401(client: TestClient, monkeypatch: MonkeyPatch) -> None:
    from fastapi import HTTPException, status
